...
export function useCollaboration(
  ...
  // Apply a remote update to the plan
  const applyRemoteUpdate = useCallback((update: PlanUpdateEvent) => {
    switch (update.type) {
      case "room":
  ...
  const broadcastUpdate = useCallback((update: Omit<PlanUpdateEvent, "timestamp" | "userId">) => {
  ...
  // Sync full plan
  const syncPlan = useCallback((newPlan: FloorPlan) => {
    if (!socketRef.current || !roomId) return;
