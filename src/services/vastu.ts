...
const IDEAL_ZONES: Record<RoomType, { best: Direction[]; neutral: Direction[]; avoid: Direction[] }> = {
  'Bedroom': {
    best: ['SW', 'S', 'W'],
    neutral: ['NW', 'N', 'E'],
    avoid: ['NE', 'SE', 'CENTER']
...
export function getDirection(x: number, y: number, cx: number, cy: number, northAngle: number): Direction {
...
export function analyzeRoomVastu(room: Room, plan: FloorPlan): VastuResult {
  ...
  if (!rules) {
    return { score: 50, status: 'average', idealDirections: [], currentDirection: dir, feedback: 'No specific rules.' };
  }
