...
5.  **Arrange**: Drag rooms into the green buildable area. Resize them using the blue handle in the bottom-right corner.
6.  **Add Furniture**: Click a room to select it. In the right sidebar (or "Properties" tab on mobile), click elements to add them. Drag them around or double-click to rotate.
7.  **Keyboard Shortcuts**: Use shortcuts for faster workflow:
