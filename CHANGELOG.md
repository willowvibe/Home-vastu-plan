...
- **Vector PDF export + watermark gate** (M-1, PR #90). Replaced the raster PNG-in-PDF Presentation Export with a true vector PDF built from `FloorPlan` primitives via jsPDF vector APIs (`pdf.rect()`, `pdf.line()`, `pdf.text()`, `pdf.circle()`). New `src/lib/exportVectorPdf.ts` with a data/render split (`buildVectorPdfOps` pure → `renderOpsToPdf` thin replay). New `src/services/entitlements.ts` with localStorage-based Pro Export override (`vp_pro_export`). Free-tier PDFs include a diagonal "VastuPlan 2D — Free Plan" watermark; Pro users get watermark-free output. The `PresentationExport` component no longer requires a `canvasRef` — all drawing comes from plan data.
- **C1 AI Proxy + Database Architecture** (PR #91). Moved Gemini + Ollama AI calls behind `server/` on Railway so API keys never reach the browser (Critical audit finding resolved). New `server/src/ai/` module with provider abstraction: `analyzeWithGemini`, `editImageWithGemini`, `analyzeWithOllama`. New `POST /api/ai/analyze` and `POST /api/ai/edit-image` endpoints behind `authenticate` middleware with AI usage logged to `public.ai_usage`. Client `src/services/gemini.ts` is now a thin fetch wrapper using Supabase auth tokens. Created 6-table `public` schema in Supabase: `users`, `plans`, `plan_shares`, `sync_queue`, `ai_usage`, `user_entitlements` — all with RLS and an auto-create trigger on `auth.users`.
- **Supabase Auth Phase 1** (M-10). Optional email/password sign-up, sign-in, password reset, and sign-out via a new `AuthContext` provider (`src/contexts/AuthContext.tsx`) and a theme-aware, accessible `AuthModal` (`src/components/AuthModal.tsx`). The auth client is a singleton in `src/lib/supabase.ts`. When `VITE_SUPABASE_URL` / `VITE_SUPABASE_ANON_KEY` are missing, the UI falls back to anonymous-only mode. Sentry user identity (`setUser`/`clearUser`) and Plausible analytics events (`USER_SIGNED_IN`, `USER_SIGNED_UP`) are wired in `App.tsx`. Includes unit tests for `AuthContext` and `AuthModal`.
- **`.prettierignore`** to skip env files, tooling/session directories (`.claude/`, `.playwright-mcp/`, `.remember/`), and `market_research/`.
- **Configurable snap grid** (G-9). The canvas background grid and drag/resize snap step are no longer hard-coded to 1 ft. `FloorPlan` stores an optional `gridSize` (default 1 ft) with a `Sidebar` selector offering 0.5, 1, 2, 3, and 5 ft (displayed in meters when `plan.unit === 'm'`). `useCanvasDrag` accepts `gridSize` and snaps to it when `snapToGrid` is on. New `DEFAULT_GRID_SIZE_FT` and `GRID_SIZE_OPTIONS_FT` live in `src/constants/geometry.ts`.
- **Vastu zone tour + hover cards** (G-10). A new compass icon in the toolbar opens a guided, step-by-step Vastu zone tour over the canvas. The tour highlights each of the nine zones in turn and shows element, ideal use, and a Vastu tip. The grid cells now render `title` tooltips with the same metadata. Zone metadata and direction math live in the new `src/constants/vastuZones.ts`; the tour overlay is `src/components/VastuTour.tsx`.
- **Indian-convention floor names** (G-15). New `formatFloorLabel(n, floorNames?)` in `src/constants/floorPlanConstants.ts` defaults to `Ground Floor`, `First Floor`, `Second Floor`, etc., while still allowing per-floor custom names via `plan.floorNames`. Call sites in `Sidebar`, `usePlanEditor`, `CanvasArea`, `App.tsx` print modal, `PresentationExport`, and `lib/complianceReport` now use the readable labels; E2E floor selectors were updated accordingly.
- **Staircase element** (G-3). `ROOM_ELEMENTS['Stairs']` in `src/constants/floorPlanConstants.ts` now includes a `{ type: 'Staircase', w: 4, h: 10 }` element, so multi-floor plans get a dedicated staircase piece. `RoomElement.tsx` renders staircases with a step-hatch pattern instead of the generic element fill. New `STAIRCASE_ADDED` analytics event fired from `usePlanEditor.addRoomElement`.
- **Plumbing overlay** (G-4). New `src/components/PlumbingOverlay.tsx` SVG layer renders water connections for kitchens and bathrooms. Wired through `usePlanEditor` (`showPlumbing`, `handleTogglePlumbing`), `CanvasArea`, `Canvas`, `App.tsx`, and a new "Overlays" section in `Sidebar`. New `PLUMBING_OVERLAY_TOGGLED` analytics event.
- **Sun path / shadow overlay** (G-5). New `src/components/SunPathOverlay.tsx` renders convex-hull shadow polygons per room based on orientation, date, time, and room height. `FloorPlan` gained an optional `latitude?: number` (defaults to New Delhi via `DEFAULT_LATITUDE` in `src/utils.ts`). Sidebar exposes date, time, and latitude controls plus a "Now" button. New sun-position and time format/parse helpers (`getSunPosition`, `formatSunTime`, `parseSunTime`, `DEFAULT_ROOM_HEIGHT_FT`, `DEFAULT_LATITUDE`) in `src/utils.ts`. New `SUN_PATH_TOGGLED` and `SUN_PATH_TIME_CHANGED` analytics events.
...
- **Compliance report PDF** (G-6, PR #70). New `ComplianceReportExport` modal reachable from the toolbar "Compliance" button. Captures the live canvas via `html-to-image` `toPng`, then builds a portrait letter PDF (`jsPDF`) with project/client/consultant metadata, overall Vastu score, AI analysis summary, a per-room compliance table, total built-up area, and the floor-plan image. Pure report builder lives in `src/lib/complianceReport.ts`.
- **Per-room cost estimation** (G-8, PR #70). Rooms now carry an optional `costPerSqFt?: number` (default `2000`). `RoomPropertiesPanel` shows each selected room's area and estimated cost in an editable card; multi-selection shows the combined cost. `Sidebar` total cost now uses `getTotalCost(plan.rooms)` and `formatCurrency()`. New cost helpers in `src/utils.ts` with unit tests.
- **Share-with-annotation mode** (G-11, PR #70). `appMode` union widened to `'edit' | 'view' | 'comment'`. A comment-enabled share link opens the plan in `comment` mode, where reviewers can drop comment pins on the canvas. `Canvas` renders pins for the current floor, supports selecting a pin, and `PropertiesPanel` shows a `CommentPropertiesPanel` card to view/edit/delete the comment (read-only in `view` mode). Comment CRUD lives in `usePlanEditor`; `analytics.ts` tracks `COMMENT_ADDED`, `COMMENT_UPDATED`, and `COMMENT_DELETED`.
- **Password-protected share links** (G-14). `src/lib/shareLink.ts` now uses the Web Crypto API (`PBKDF2` + `AES-GCM`) to encrypt shared plans. `generateProtectedShareLink(plan, analysis, mode, password)` produces a URL whose `plan` parameter starts with `enc:v1:`. Opening such a link prompts for the password, decrypts the payload, and loads the plan into `view` or `comment` mode. The plain `generateShareLink` path remains unchanged. New `encryptPlan` / `decryptPlan` helpers and 6 new tests in `src/lib/shareLink.test.ts`.
- **Multi-user undo across collaboration boundary** (G-1). `useCollaboration` is now wired into `usePlanEditor`. Remote edits received over the socket are committed to the local undo history via the new `commitHistoryUpdate` method in `useFloorPlan`, so collaborators' changes can be undone (and redone) locally. Local users can broadcast `undo-request` / `redo-request` events with the new `requestUndo()` / `requestRedo()` helpers; peers in the same room receive `undo-requested` / `redo-requested` and apply their own undo/redo. `App.tsx` now renders `<CollaborationPanel>`. New tests in `useFloorPlan.test.ts`, `useCollaboration.test.ts`, and `usePlanEditor.test.ts`.
- **`@testing-library/user-event` keyboard tests for `useKeyboardShortcuts`** (G-13). `src/hooks/useKeyboardShortcuts.test.tsx` now uses `userEvent.setup()` instead of hand-rolled `KeyboardEvent` dispatch. 21 tests cover Ctrl+Z/Y/Shift+Z, Delete, Ctrl+D, r/R, g, zoom, nudge, shortcut help, and input/textarea/contentEditable gating.
...
- **Mobile canvas overflow**. The centered canvas wrapper in `src/App.tsx` changed from `items-center` to `items-start md:items-center` so mobile users see the left edge of the plan instead of an overflowing, inaccessible canvas. The `VastuTour` info card was split off into a viewport-centered `fixed` overlay so the tour's highlight stays anchored to the canvas while the explanation stays on-screen.
- **Sentry `process.env` in browser bundle**. `src/services/sentry.ts` now uses `import.meta.env.DEV` instead of `process.env.NODE_ENV`, removing a build-time `define` dependency that broke in pure-browser contexts. New regression tests in `src/services/sentry.test.ts` delete `globalThis.process` and verify `captureError` / `captureMessage` do not throw.
- **E2E port alignment** (Q-4). `vite.config.ts` now reads `process.env.PORT`, `package.json` no longer hardcodes `--port=3000` on the `dev` script, and `playwright.config.ts` / `.github/workflows/ci.yml` default to `3000`. This fixes the `Timed out waiting 60000ms from config.webServer` failure caused by the dev server and Playwright waiting on different ports.
